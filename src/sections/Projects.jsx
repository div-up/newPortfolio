...
export default function Projects() {
  ...
  return (
    <section id="projects" ref={ref} className="relative py-32" style={{ paddingLeft: PX, paddingRight: PX, background: 'var(--section-bg)' }}>
      ...
      <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.7, delay: 0.1 }}
        ...
        <span style={{ color: 'var(--text)' }}>PROJECTS</span><br/>
        <span style={{ color: 'var(--accent)' }}>I'M PROUD OF</span>
      </motion.div>
      <div className="flex flex-col gap-0">
        {PROJECTS.map((p, i) => (
          <motion.a key={p.num} href={p.link}
            ...
            className="group border-t py-8 md:py-10 grid md:grid-cols-[80px_1fr_auto] gap-6 items-center"
            style={{ textDecoration: 'none', transition: 'background 0.3s', borderTopColor: 'var(--section-border)' }}
            onMouseEnter={e => e.currentTarget.style.background = 'var(--card-bg)'}
            onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
            <div style={{ fontFamily: 'Bebas Neue,sans-serif', fontSize: 'clamp(14px,1.5vw,18px)', color: 'var(--muted-strong)', letterSpacing: '0.1em' }}>{p.num}</div>
            <div>
              <div style={{ fontFamily: 'Bebas Neue,sans-serif', fontSize: 'clamp(28px,3.5vw,52px)', color: 'var(--text)', lineHeight: 1, letterSpacing: '0.03em', transition: 'color 0.3s' }}
                className="group-hover:text-[var(--accent)]">{p.title}</div>
              <div style={{ fontFamily: 'Space Grotesk,sans-serif', fontSize: 'clamp(12px,1vw,14px)', color: 'var(--muted)', marginTop: '0.5rem', lineHeight: '1.6' }}>{p.desc}</div>
            ...
            <div style={{ fontFamily: 'Space Grotesk,sans-serif', fontSize: '11px', letterSpacing: '0.15em', color: 'var(--muted-strong)' }}>{p.year}</div>
          </motion.a>
        ))}
