...
export default function Skills() {
  ...
  return (
    <section id="skills" ref={ref} className="relative py-32" style={{ paddingLeft: PX, paddingRight: PX, background: 'var(--section-bg)' }}>
      ...
      <motion.div initial={{ opacity: 0, x: -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.7, delay: 0.1 }}
        ...
        <span style={{ color: 'var(--text)' }}>MY SKILLS &amp;</span><br/>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-px" style={{ background: 'var(--section-border)' }}>
        {CATS.map((cat, i) => (
          <motion.div key={cat.name}
            ...
            <div style={{ fontFamily: 'Bebas Neue,sans-serif', fontSize: 'clamp(20px,2vw,28px)', color: 'var(--accent)', letterSpacing: '0.05em', marginBottom: '1.5rem' }}>{cat.name}</div>
            <ul className="flex flex-col gap-3">
              {cat.skills.map((s, j) => (
                <motion.li key={s}
                  initial={{ opacity: 0, x: -20 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.4, delay: 0.3 + i * 0.05 + j * 0.05 }}
                  className="flex items-center gap-3"
                  style={{ fontFamily: 'Space Grotesk,sans-serif', fontSize: 'clamp(12px,1.1vw,14px)', color: 'var(--muted)' }}>
